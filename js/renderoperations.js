function renderMultiplyRow() {
    var row =
        Number(document.getElementById("rowmultiplicationdiv_row").value) - 1;
    try {
        var factor = Number(
            document.getElementById("rowmultiplicationdiv_factor").value
        );
        if (!isNaN(factor)) {
...
function renderAddRows() {
    ...
    try {
        var factor = Number(
            document.getElementById("rowadditiondiv_factor").value
        );
        if (!isNaN(factor)) {
...
function renderMultiplyColumn() {
    ...
    try {
        var factor = Number(
            document.getElementById("colmultiplicationdiv_factor").value
        );
        if (!isNaN(factor)) {
...
function renderAddColumns() {
    var column1 =
        Number(document.getElementById("coladditiondiv_col1").value) - 1;
    var column2 =
        Number(document.getElementById("coladditiondiv_col2").value) - 1;
    try {
        var factor = Number(
            document.getElementById("coladditiondiv_factor").value
        );
        if (!isNaN(factor)) {
